...
2. 物体创建
3. 物体检查，不存在则自动创建
4. 网络物体初始化/重新创建事件。创建时提供了string类型的参数可传入，**初始化/重新创建事件在Start之前被调用**
...
将代码生成改为织入IL指令表，降低代码侵入性并提高性能。

兼容对长信息的分片。当前遇到超出1400字节的消息，为了防止传输时发生分片，会拒绝传输。
...
发送的各种数据大部分都是较短的数据片段，比如一次远程函数调用、一次网络事件，且大部分无顺序要求。为了减少消息的发送次数，需要将短的数据片段拼成长数据片段。
...
消息在拼接时，会在消息间插入三个连续的0xFE，每个消息片段中，最前面的消息类型不包含0xFE，最后面的**消息长度经过特殊编码，不会包含0xFE**，从而实现了消息片段拼接后仍可被划分。这种设计，即使拼接后的数据片段在传输时被切割、首/尾部丢失，也可以划分出可用片段。中间被切割的小片段会在消息重传中再次传输，尝试避免被划分或部分丢失

### 消息重传

消息重传划分了四种消息重要性的级别：

不可靠传输：不进行校验，适用于实时性要求高且发送次数多的消息，如玩家位置同步

尽力交付传输：不进行校验，会发送多次，适用于重要且有实时性要求的消息，如玩家可大量使用的道具

可靠传输：进行校验，网络正常时绝大部分情况下可成功传输。适用于实时性要求不高且非常重要的消息，如战斗开始

有序传输：进行校验，未校验的数据会阻碍之后数据的传输。适用于无实时性要求且重要且需要有序传输的消息，如客户端按照顺序完成初始化。**若中途有数据未成功传输，则会立即丢失并执行下个数据片段的传输，因而不适用于无标识的消息，以免中间缺失个别消息导致消息全部混乱**

### 插件功能结构

Transport：信息传输模块，被设计成了可替换的模块，从而适配更多的平台

**E**vents：对信息传输进行封装，提供基础的连接事件，如连接到服务器、客户端加入连接等

**N**et Object：网络物体管理，对每个网络物体在所有客户端中分配相同的标识，实现远程函数调用、物体生成、物体检测重建

**S**erver Room：服务器创建多个房间，将来自客户端的广播只发送至同房间的其它客户端，便于玩法上划分房间

Game Play：绑定事件、配置信息、录入网络物体，即可实现可划分房间且高适配性的游戏

### Server-Connection-Client结构

服务器本身为Server，服务器会持有多个Connection，每个对应一个客户端。Connection位于服务器端，封装了和Client的消息收发。

### 代码生成

借助Roslyn语义分析，找到所有需要进行代码生成的类。之后扫描每个函数，找到有Rpc标记的函数，记录并生成对应的代码。

### 时间同步

来自服务器的心跳检测数据包中，会包含服务器的时间。客户端收到该数据包时，可以截取出服务器时间，之后发送一个回应数据包，将服务器时间放入数据包。服务器接收到回应后，比较发送和接收数据包的时间，即为一次数据往返的时间。×0.5后即为延迟，随下一次心跳检测数据包一起发给客户端。客户端接收后记录延迟。

### 网络物体管理

对于服务器中的每一个房间，会有一个循环的物体id源。新客户端加入时会立即将该源同步至新客户端。创建物体时，服务器和同房间的客户端都会将id源进行偏移，偏移量为物体的网络行为脚本数。

对于各类管理器，推荐直接放入场景，且不重复加载

特别的，当物体id到达上限后，id不会重置为0而是100。因为可能有网络游戏管理器不会被摧毁或长期存在，为了防止id发生重复，id被重置成了100。这里的假设是管理器不会占用超过100个id，且非管理器不会长期存在导致id增量了32767个后发生重复。
