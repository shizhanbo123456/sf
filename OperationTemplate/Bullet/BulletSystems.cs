...
public static class BulletSystemCommon
{
    ...
    public static float VectorToAngle(Vector3 dt)
    {
...
public static class BulletFollowSystem
{
    ...
    public static void RegistObject(Bullet obj, float radius, float lifeTime)
    {
        if (Collection.Count == 0)
            BulletManager.SystemUpdateLoop += Update;
        obj.transform.position = BulletSystemCommon.CurrentShooter.transform.position;
        obj.transform.localScale *= radius;
        Collection.Add(obj, new FollowInfo(Time.time+lifeTime,BulletSystemCommon.CurrentShooter.Camp,BulletSystemCommon.CurrentShooter.ObjectId));
        obj.ReleaseBulletSystemReference = ReleaseReference;
    }
    private static void ReleaseReference(Bullet b)
    {
...
public static class BulletOrbitSystem
{
    private struct OrbitInfo
    {
        ...
        public OrbitInfo(float destroyTime, float radoffset, float radspeed, float orbitRadius, float lifetime)
        {
            this.destroyTime = destroyTime;
        ...
        float degreePerSec,float angleOffset)
    {
        ...
        Collection.Add(obj, new OrbitInfo(Time.time + lifetime, angleOffset * Mathf.Deg2Rad,
            degreePerSec * Mathf.Deg2Rad, orbitRadius,lifetime));
        obj.ReleaseBulletSystemReference = ReleaseReference;
    }
    private static void ReleaseReference(Bullet b)
    {
...
public static class BulletProjectileSystem
{
...
public static class BulletProjectileAimSystem
{
